...
# Add parent directory to path to allow imports from search_service
...
class RestaurantUpdater:
    ...
    def search_restaurants(self, query: str) -> List[Dict]:
        ...
        try:
            ...
            for place in places_result.get('results', []):
                ...
                self.logger.info(f"Found restaurant: {restaurant['name']}")
    ...
    def _get_cuisine_type(self, place_details: Dict) -> List[str]:
        """Extract cuisine type from place details."""
        try:
            # Try to get cuisine from types first
            types = place_details.get('types', [])
            cuisine_types = [t.replace('_cuisine', '').title() for t in types if 'cuisine' in t]
            
            # Try to get from categories if available
            categories = place_details.get('categories', [])
            if categories:
                cuisine_types.extend([cat.title() for cat in categories])
            
            # Remove duplicates and empty strings
            cuisine_types = list(set(filter(None, cuisine_types)))
...
async def main():
    try:
        ...
        # Get restaurants from command line arguments
        restaurants = sys.argv[1:]
        if not restaurants:
            print("Please provide restaurant names as arguments")
            return
            
        all_restaurants = []
        print(f"\nSearching for {len(restaurants)} restaurants in London...")
        
        # Search for each restaurant
        for restaurant in sorted(restaurants):
            print(f"\nSearching for: {restaurant}")
            # Add "London" to the search query for better results
            query = f"{restaurant} London"
            restaurants = updater.search_restaurants(query)
            if restaurants:
                all_restaurants.extend(restaurants)
                print(f"Found {len(restaurants)} result(s) for {restaurant}")
            else:
                print(f"No results found for {restaurant}")
        
        if all_restaurants:
            # Update database
            await updater.update_database(all_restaurants)
            print(f"\nSuccessfully processed {len(all_restaurants)} restaurants")
        else:
